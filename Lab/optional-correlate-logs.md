...
4. クエリにラベルフィルターを追加して、疑わしいサービス `web_app_3` に絞り込みます。
    ...
    疑わしい「のこぎり状のパターン」に気づきましたか？同時接続ユーザーのリクエストが急減している原因を突き止めましょう。
...
2. 新しく追加された（右側の）パネルには、同じ Prometheus クエリが表示されています。右側パネルのデータソースの選択を、**LokiCorrelation** データソースに変更します。

    Grafana が Prometheus のラベル選択を認識し、同じラベルを Loki のログクエリに適用するため、関連する最新のログがすぐに表示されることが分かります。
...
4. 左側の Prometheus パネルで、マウスをクリックしてドラッグし、急減が発生した時間範囲を選択します。
   ...
   <img width="1070" height="844" alt="image" src="https://github.com/user-attachments/assets/75b1d709-489e-4914-90c2-38ada8646441" />
    ...
    Loki パネルの時間範囲が同期され、新しく選択した期間のログが再読み込みされることが分かります。
